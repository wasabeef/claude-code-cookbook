---
description: "実装前の計画立案モードを起動して詳細な実装戦略を策定する。「計画を立てて」「どう進める？」「実装戦略は？」「プランニングして」「設計を考えて」「アプローチを提案して」「どう実装すべき？」などで起動。構造化された計画で効率的な開発を支援。"
allowed-tools:
  - Read
  - Grep
  - Glob
...
- `ExitPlanMode` ツールで計画を提示
- **重要**: ツールの戻り値に関わらず、必ずユーザーの明示的承認を待つ
- 承認なしでの実装開始は禁止
- 計画の修正・調整が可能
- 承認後にのみ TaskCreate でタスク管理を開始
...
- **タスク分解**: 実装可能な単位への細分化
- **進捗追跡**: TaskUpdate による状態管理
- **品質保証**: テスト戦略と検証方法
- **承認プロセス**: ExitPlanMode での計画提示と明示的承認待機
