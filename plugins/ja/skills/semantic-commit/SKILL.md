---
description: "大きな変更を意味のある最小単位に分割してセマンティックなコミットメッセージと共に順次コミットする。「コミットして」「変更をコミットして」「分割してコミット」「セマンティックにコミット」「意味のある単位でコミット」「論理的にコミットして」「変更を整理してコミット」「適切に分けてコミット」などで起動。"
allowed-tools:
  - Bash(git *)
---
