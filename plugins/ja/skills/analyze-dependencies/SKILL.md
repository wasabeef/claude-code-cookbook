---
description: "プロジェクトの依存関係を分析しアーキテクチャの健全性を評価する。「依存関係を分析して」「循環依存を検出して」「アーキテクチャの問題は？」「モジュールの依存を確認」「dependency を調べて」「結合度をチェックして」「レイヤー違反を見つけて」などで起動。依存関係マトリックス、ファンイン/ファンアウト、循環依存、レイヤー違反を検出し、修正優先度付きの改善提案を生成する。"
allowed-tools:
  - Read
  - Grep
  - Glob
