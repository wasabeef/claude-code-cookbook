---
description: "Core Web Vitals に基づくパフォーマンス分析と UX スコア算出。「パフォーマンスを分析して」「速度を改善したい」「Core Web Vitals をチェック」「ページ速度を調べて」「LCP を改善」「パフォーマンス問題を特定して」などで起動。"
allowed-tools:
  - Read
  - Grep
  - Glob
