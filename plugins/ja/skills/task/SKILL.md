---
description: "専用エージェントを起動して複雑な検索・調査・分析タスクを自律的に実行する。「Task で調査して」「調べて」「分析して」「検索して」「情報収集して」「バックグラウンドで実行」などで起動。複数のツールを組み合わせた大規模な情報処理で、コンテキスト効率性を重視。"
allowed-tools:
  - Read
  - Grep
  - Glob
...
```bash
# 技術トレンド調査
「2026 年の JavaScript フレームワーク動向を Task で調査して」
