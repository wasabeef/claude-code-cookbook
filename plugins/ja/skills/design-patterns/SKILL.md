---
description: "コードベースに適用可能なデザインパターンを提案し SOLID 原則の遵守状況を評価する。「デザインパターンを提案して」「適用できるパターンは？」「SOLID 原則をチェック」「設計パターンで改善して」「アンチパターンを検出して」「リファクタリングパターンは？」「コードの設計を評価して」などで起動。"
allowed-tools:
  - Read
  - Grep
  - Glob
