---
description: "구현 전 계획 수립 모드로 전략 책정. 「계획 세워줘」「어떻게 진행할까?」「구현 전략은?」「플래닝해줘」등으로 기동."
allowed-tools:
  - Read
  - Grep
  - Glob
...
- `ExitPlanMode` 도구로 계획 제시
- **중요**: 도구의 반환 값과 관계없이 반드시 사용자의 명시적 승인 대기
- 승인 없는 구현 시작 금지
- 계획의 수정·조정 가능
- 승인 후에만 TaskCreate 로 작업 관리 시작
...
- **진행 추적**: TaskUpdate 를 통한 상태 관리
- **품질 보증**: 테스트 전략과 검증 방법
- **승인 프로세스**: ExitPlanMode 로 계획 제시 및 명시적 승인 대기
...
- `ExitPlanMode` 도구의 반환 값은 신뢰하지 않음
